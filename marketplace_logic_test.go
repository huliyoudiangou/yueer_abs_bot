...
func TestMarketplaceSellerNoticeShowsBuyerAndMaskedSecret(t *testing.T) {
	fullCode := "RENEW-SECRET-FULL-CODE"
	preview := "RENE…CODE"
	result := marketplacePurchaseResult{
		Listing: MarketplaceListing{Name: "30 天续期卡", ListingType: marketplaceTypeSecret},
		BuyerID: 123456789, BuyerName: "测试买家", Quantity: 1,
		PurchaseIDs: []uint{88}, PurchasePreviews: []string{preview},
		GrossAmount: 100, FeeAmount: 5, SellerAmount: 95,
	}
	text := marketplaceSellerDealNoticeText(result)
	for _, want := range []string{"测试买家", "123456789", "订单 #88", preview} {
		if !strings.Contains(text, want) {
			t.Fatalf("seller notice missing %q: %s", want, text)
		}
	}
	if strings.Contains(text, fullCode) {
		t.Fatal("seller notice must not expose full code")
	}

	orderText := formatMarketplacePurchaseLine(MarketplacePurchase{ListingID: 7, ItemName: "30 天续期卡", Quantity: 1, BuyerID: 123456789, SellerID: 9, CodePreview: preview}, false)
	if !strings.Contains(orderText, preview) {
		t.Fatalf("seller order missing masked preview: %s", orderText)
	}
	if strings.Contains(orderText, fullCode) {
		t.Fatal("seller order must not expose full code")
	}
}

func TestMarketplaceInventorySellerNoticeHasNoSecretSection(t *testing.T) {
	text := marketplaceSellerDealNoticeText(marketplacePurchaseResult{
		Listing: MarketplaceListing{Name: "聚气丹", ListingType: marketplaceTypeInventory},
		BuyerID: 1, BuyerName: "买家", Quantity: 1, PurchaseIDs: []uint{2},
	})
	if strings.Contains(text, "已售卡密") {
		t.Fatalf("inventory seller notice must not show secret section: %s", text)
	}
}
